...
# volume mapping.
export NMCP_LOCAL_BACKUP_LOCATION=
...
export NMCP_REMOTE_BACKUP_LOCATION=

# Overrides the container path where backups are saved.  If NMCP_REMOTE_BACKUP_LOCATION can be set to the full backup
# location, this can be left empty and the container default will be used.
export NMCP_REMOTE_BACKUP_PATH=

# The following are analogous to remote backup location/path.  If the additional redirection is not required for
# permissions or other reasons, NMCP_RESTORE_PATH can be left empty and the default will be used.
export NMCP_RESTORE_LOCATION=

export NMCP_RESTORE_PATH=
...
# the host as read/write.  Therefore only NMCP_LOCAL_BACKUP_LOCATION is required and this location is mapped to
# /opt/data in the container and backups will be to subdirectories in ${NMCP_LOCAL_BACKUP_LOCATION}/backup/.
#
...
#
# In the example above, NMCP_REMOTE_BACKUP_PATH would be set to /mnt/foo to allow the volume mapping to the container (it
# will be mapped to /opt/data) and and NMCP_REMOTE_BACKUP_PATH (used to override BACKUP_PATH in the container) would be
